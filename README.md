...
学习模式默认生成一个单文件系统化专题笔记，并保存到 Obsidian 当前真实 vault 的正式知识分类目录，而不是只放到 Codex 工作区或生成输出区。
...
解析到 vault 后，优先匹配这些既有目录模式：

```text
02-AI与数字化/智能体与技能/        # AI、Agent、技能、Harness、MCP
02-AI与数字化/AI事项知识库/        # AI 事项、AI 运营知识库
03-CRM与客户运营/                  # CRM、客户运营、业务系统
01-经营管理/                       # 战略、组织、管理方法
05-需求与产品/                     # 产品、PRD、需求、UX
09_生成输出/学霸/                  # 草稿、测试、失败报告、中间产物
```
...
- 正式学习笔记应进入匹配主题的 Obsidian 分类目录；`09_生成输出` 只作为草稿、测试或失败报告区。
